...
This sample app demonstrates how to use a Microsoft Teams bot to chat with your data using natural language in the workflow.
...
- [Adaptive Cards](https://adaptivecards.io), are an open card exchange format enabling developers to exchange UI content commonly and consistently.
...
> [!TIP]
> Use this approach if you don't have access to a Microsoft 365 tenant, are unable to use a Dev Tunnel, are unable to sideload apps into your tenant, or are unable to provision Microsoft Entra ID or Bot Framework resources required to create a bot.
...
1. Open the sidebar in VS Code, select the `Run and Debug` panel
...
> [!NOTE]
> An Azure subscription is not required to run this sample locally, however, you will need a [Microsoft 365 tenant](https://developer.microsoft.com/microsoft-365/dev-program?WT.mc_id=m365-00000-garrytrinder) that has sideloading enabled.
...
1. Open the sidebar in VS Code, select the `Teams Toolkit` icon
